...
# Accepted dice must remain close to their previous positions during a
# selective retry. Red/uncertain dice are expected to move.
...
FEEDBACK_DIRECTORY = Path("dataset_feedback/d8")
...
def locked_candidate_mapping(
    previous: AggregationResult,
    current: AggregationResult,
) -> dict[int, int]:
    """
    Match previously accepted dice to their current candidates by position.

    Green dice are treated as locked. The user may reposition only red dice.
    """
    locked_slots = [
        index
    ...
    for slot_index in locked_slots:
        previous_point = np.asarray(
            previous.representative_candidates[
                slot_index
            ].centroid,
            dtype=np.float32,
        )
    ...
    if missing:
        ...
        raise RuntimeError(
            "Could not find the previously accepted dice at their "
            f"old positions: {missing_text}. Keep green dice still "
            "and move only red dice."
        )
...
def merge_retry_result(
    ...
    """
    Preserve accepted predictions and replace only uncertain predictions.

    Accepted dice are matched by centroid. Remaining current candidates are
    assigned to the previous uncertain slots in reading order.
    """
    ...
    locked_mapping = locked_candidate_mapping(
        previous,
        current,
    )
    ...
    if len(retry_current_indices) != len(
        uncertain_slots
    ):
        raise RuntimeError(
            "Could not separate the repositioned dice from the "
            "locked dice. Move red dice farther from green dice "
            "and retry."
        )
    ...
    merged_candidates: list[
        collector.Candidate | None
    ] = [None] * len(
        previous.predictions
    )

    for slot_index, current_index in (
        locked_mapping.items()
    ):
        merged_predictions[slot_index] = (
            previous.predictions[
                slot_index
            ]
        )
    ...
    # Both lists are in stable reading order. Identity among uncertain dice
    # does not affect the total, but their persistent slot numbers remain
    # useful to the user.
    ...
    if any(
        ...
        raise RuntimeError(
            "Internal retry merge error: a prediction slot "
            "was not filled."
        )
    ...
    if any(
        ...
        raise RuntimeError(
            "Internal retry merge error: a candidate slot "
            "was not filled."
        )
    ...
    return AggregationResult(
        ...
        representative_tray=(
            current.representative_tray
        ),
        representative_mask=(
            current.representative_mask
        ),
        representative_candidates=[
            candidate
            for candidate in merged_candidates
        ...
        representative_split_count=(
            current.representative_split_count
        ),
...
def reader_poll_command() -> str | None:
    """
    Read B/C/F/N/R/S/Q from either an OpenCV window or the Windows console.
    """
    ...
    if (
        msvcrt_module is not None
        and msvcrt_module.kbhit()
    ):
        character = (
            msvcrt_module.getwch()
        )
...
def parse_die_indices(
    ...
    """
    Parse user-facing one-based die numbers such as "2 5", "2,5", or "2-4".
    """
...
def prompt_for_wrong_dice(
    ...
    print(
        "Enter every die whose displayed value is wrong."
    )
    print(
        "Press Enter if none of the displayed values are wrong."
    )
    ...
    while True:
        ...
        try:
            return parse_die_indices(
                response,
                len(result.predictions),
            )
        except (TypeError, ValueError) as error:
            print(
                f"{error} Examples: 2 5, 2,5, or 2-4."
            )


def prompt_for_true_value(
    die_number: int,
    predicted_value: int,
) -> int:
    while True:
        response = input(
            f"True value for die {die_number} "
            f"(displayed {predicted_value}): "
        ).strip()

        try:
            value = int(response)
        except ValueError:
            print("Enter a d8 value from 1 through 8.")
            continue
        ...
        if value == predicted_value:
            print(
                "That matches the displayed value. "
                "Remove this die from the wrong-dice list, "
                "or enter its different true value."
            )
            continue
...
def save_feedback_example(
    ...
    """
    Save one representative full masked crop.

    Filenames retain one shared collector-style session prefix across retries,
    ensuring future session-grouped splits keep related examples together.
    """
...
def review_predictions(
    ...
    """
    Apply the user's truth review before any repositioning.

    Any model-uncertain die not flagged as wrong becomes accepted and locked.
    Flagged dice remain red and are saved with their true labels.
    """
    if result.reviewed:
        print(
            "This capture has already been reviewed."
        )
        return result

    wrong_indices = prompt_for_wrong_dice(
        result
    )
    true_values: dict[int, int] = {}

    for die_index in sorted(
        wrong_indices
    ):
        true_values[die_index] = (
            prompt_for_true_value(
                die_index + 1,
                result.predictions[
                    die_index
                ].value,
    ...
    revised_predictions: list[
        D8Prediction
    ] = []
    saved_examples: list[Path] = []

    for die_index, prediction in enumerate(
        result.predictions
    ):
        if die_index in wrong_indices:
            ...
            saved_examples.append(
                save_feedback_example(
                    ...
                    true_value=true_value,
                    feedback_type=(
                        "user_flagged_misclassification"
                    ),
                    session_id=session_id,
                    attempt=attempt,
                    bundle_55=bundle_55,
        ...
        if not prediction.accepted:
            # This was a low-confidence/model-disagreement case, but the user
            # verified that its displayed value was correct.
            saved_examples.append(
                save_feedback_example(
                    result=result,
                    die_index=die_index,
    ...
    if saved_examples:
        print(
            "\nSaved feedback examples:"
        )

        for path in saved_examples:
            print(f"  {path.resolve()}")
    else:
        print(
            "\nNo feedback images were needed for this result."
        )
    ...
    if flagged_numbers:
        print(
            "Reposition only these red dice: "
            + ", ".join(
                flagged_numbers
            )
        )
        print(
            "Keep every green die still, then press R."
        )
    else:
        print(
            "All displayed values are now accepted."
        )
...
def annotate_predictions(
    ...
    for index, (
        ...
        if prediction.flagged_wrong:
            state_text = (
                f"WRONG truth={prediction.true_value}"
            )
        elif prediction.user_confirmed:
            state_text = "CONFIRMED"
        elif prediction.accepted and not all_accepted:
            state_text = "LOCKED"
        else:
            state_text = ""
    ...
    if all_accepted:
        ...
        second_line = (
            "Press F if any displayed value is wrong; "
        ...
        if result.reviewed:
            ...
            second_line = (
                "Do not move dice yet. Press F to identify "
                "which displayed values are wrong."
            )

        status_color = (0, 0, 255)

    return add_result_banner(
        output,
        first_line,
        second_line,
        status_color,
...
def build_prediction_sheet(
    ...
    for index, prediction in enumerate(
        ...
        if prediction.flagged_wrong:
            state_text = (
                f"WRONG -> {prediction.true_value}; MOVE + R"
            )
        elif prediction.user_confirmed:
            state_text = "USER CONFIRMED; KEEP STILL"
            ...
            state_text = "REPOSITION + R"
        else:
            state_text = "REVIEW WITH F"
...
def display_and_report_result(
    ...
    for index, prediction in enumerate(
        ...
        if prediction.flagged_wrong:
            ...
            state = (
                "user-confirmed correct / keep still"
            )
        elif prediction.accepted:
            state = (
                "accepted / keep still"
            ...
            state = "UNCERTAIN / reposition"
        else:
            state = "UNCERTAIN / review with F"
    ...
    print(f"  Provisional total: {total}")

    if all_accepted:
        print("  Result accepted.")
        print(
            "  Press F if any displayed value is wrong; "
            "otherwise remove all dice and press B."
        )
    elif result.reviewed:
        wrong_indices = [
            str(index)
        ...
        print(
            "  Keep all green dice still, then press R."
        )
    else:
        print(
            "  Do not move the dice yet."
        )
        print(
            "  Press F and identify which displayed "
            "values are actually wrong."
        )
...
def main() -> int:
    ...
    print("Icy Dice d8 live reader v1.2")
    print("----------------------------")
    print(
    ...
    print(
        "  C  capture and classify the initial roll"
    )
    print(
        "  F  review displayed values and flag actual errors"
    )
    print(
        "  R  retry only user-flagged red dice after repositioning"
    )
    print(
    ...
    def capture_and_classify() -> AggregationResult | None:
        print(
        ...
        try:
            return aggregate_selected_burst(
                selection=selection,
                background=background,
                request=request,
                bundle_55=bundle_55,
        ...
        while True:
            ...
            if command == "c":
                if result_active:
                    print(
                        ...
                        "Use F to review it, R after review, "
                        "or remove all dice and press B."
                    )
                    continue
                ...
                if feedback_session_id is None:
                    feedback_session_id = datetime.now().strftime(
                        "%Y%m%d_%H%M%S_%f"
                    )
                feedback_attempt += 1
            ...
            if command == "f":
                if (
                    ...
                    print(
                        "No classified result is available for review."
                    )
                    continue
                ...
                if last_result.reviewed:
                    print(
                        "This capture has already been reviewed. "
                        "Move only red dice and press R, or "
                        "remove all dice and press B."
                    )
                    continue
                ...
                print(
                    "\nClick PowerShell to review the result."
                )

                reviewed_result = review_predictions(
                    result=last_result,
                ...
                last_result_sheet = result_sheet
                last_rectified = (
                    reviewed_result.representative_tray
                )
                last_mask = (
                    reviewed_result.representative_mask
                )
                last_main_image = annotated
                result_active = True
                background_ready = False
            ...
            if command == "r":
                if (
                    ...
                    print(
                        "No result is waiting for a selective retry."
                    )
                    continue

                if not last_result.reviewed:
                    print(
                ...
                if background is None:
                    print(
                        "Retry is unavailable because the "
                        "empty-tray background is missing."
                    )
                    continue
                ...
                print(
                    "  Keep every green die in its current position."
                )
                print(
                    "  Reposition only the user-flagged red dice."
                )
                ...
                if current_result is None:
                    print(
                        "Leave the dice in place and press R "
                        "to try the selective retry again."
                    )
                    continue
                ...
                try:
                    ...
                    print(
                        f"Selective retry rejected: {error}"
                    )
                    print(
                        "Restore green dice to their prior "
                        "positions, move only red dice, and "
                        "press R again."
                    )
                    continue
                ...
                last_rectified = (
                    merged_result.representative_tray
                )
                last_mask = (
                    merged_result.representative_mask
                )
                last_main_image = annotated
                result_active = True
                background_ready = False
