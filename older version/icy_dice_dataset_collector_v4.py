...
def burst_progress_image(
    ...
    cv2.rectangle(output, (0, 0), (OUTPUT_WIDTH, 70), (0, 0, 0), -1)
    cv2.putText(
        output,
        (
...
def main() -> int:
    print("Icy Dice dataset collector v4")
    print("-----------------------------")
    print("When an OpenCV window asks for text input, click PowerShell")
    print("before typing the roll or visible markings.\n")
    ...
    candidate_window = "Icy Dice - Dataset Collector v4"
    mask_window = "Icy Dice - Foreground Mask"
    burst_window = "Icy Dice - Burst Capture"
    ...
    cv2.resizeWindow(candidate_window, 1000, 670)
    cv2.resizeWindow(mask_window, 900, 600)  #
    cv2.resizeWindow(burst_window, 1000, 670)  #
    cv2.resizeWindow(verify_window, 1000, 670)
        ...
        while True:
            ...
            if marker_count == 4:
                ...
                if background_ready and background is not None:
                    ...
                    camera_status = (
                        f"4 markers | target {request.expression} | "
                        "press C after dice stop"
                    )
            ...
            if command == "c":
                ...
                print(
                    f"\nCapturing approximately {BURST_DURATION_SECONDS:.1f} "
                    f"second of video for {request.expression}..."
                )
                ...
                cv2.imshow(
                    crop_window,
                    build_contact_sheet(candidate_images),
                )
                cv2.waitKey(1)
                ...
                print(
                    "\nClick PowerShell to verify the markings "
                ...
                last_annotated = frozen_annotated
